...
### UI (미완료)
- [ ] ⬜ **[P2]** 검색창 UI (닉네임 입력)
- [ ] ⬜ **[P2]** 검색 결과 유저 카드 목록 표시 (아바타, 닉네임, 게임 수)
- [ ] ⬜ **[P2]** 유저 카드 클릭 → 공개 프로필 페이지 이동

### Supabase 연동 (미완료)
- [ ] ⬜ **[P2]** 닉네임으로 유저 검색 — `supabase.from('profiles').select('*').ilike('nickname', '%query%')`
- [ ] ⬜ **[P2]** 공개 프로필 페이지 (`/:userId`) 구현 — 해당 유저의 라이브러리·통계 표시
