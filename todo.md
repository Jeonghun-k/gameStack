...
- [ ] ⬜ `games` 테이블 — 본인 데이터만 CRUD 허용
- [ ] ⬜ `comments` 테이블 — 조회는 전체, 작성/삭제는 본인만
- [ ] ⬜ `ratings` 테이블 — 본인 데이터만 CRUD 허용
- [ ] ⬜ `profiles` 테이블 — 조회는 전체, 수정은 본인만
...
- [ ] ⬜ `src/lib/supabase.js` 작성 — createClient 초기화
- [ ] ⬜ `src/context/AuthContext.jsx` 작성
  - Google OAuth 로그인 함수
  - 로그아웃 함수
  - 세션 상태 구독 (`onAuthStateChange`)
  - 전역 `user` 상태 제공
- [ ] ⬜ **[P0]** Google 소셜 로그인 버튼 연결 (LandingPage)
- [ ] ⬜ **[P0]** 로그아웃 버튼 연결 (Sidebar)
- [ ] ⬜ **[P0]** 새로고침 후 세션 복원 (AuthContext에서 처리)
...
## 9. LFG — 팀원 모집 (PRD §5-6) — 담당: 팀원 C

### UI (완료)
- [x] ✅ **[P2]** 게시글 목록 표시
- [x] ✅ **[P2]** 타입 필터 (Co-op / Party / Casual / Hunt / Guide)
- [x] ✅ **[P2]** 모집 게시글 작성 모달 UI (게임명, 타입, 설명)
- [x] ✅ **[P2]** 참여하기 버튼 UI

### Supabase 연동 (미완료)
- [ ] ⬜ **[P2]** LFG 테이블 설계 및 생성 (PRD에 미포함 → 추가 설계 필요)
- [ ] ⬜ **[P2]** 게시글 목록 조회 — Supabase SELECT
- [ ] ⬜ **[P2]** 게시글 작성 — Supabase INSERT
- [ ] ⬜ **[P2]** 참여하기 기능 — Supabase UPDATE or 별도 테이블
